...
// todo controle com nome e nenhum painel por cima da barra. Qualquer painel novo que
// quebre isso quebra a suíte.

import assert from "node:assert/strict";
import { after, before, describe, test } from "node:test";
import { Navegador, encontrarChromium, type Pagina } from "../lib/cdp.ts";
import { ALVOS_WCAG_JS, CAMINHO_JS } from "../lib/tela.ts";
...
${ALVOS_WCAG_JS}
  ...
  const recortado = (el) => { for (let n = el.parentElement; n; n = n.parentElement) { const cs = getComputedStyle(n); if (cs.overflowX === "visible" && cs.overflowY === "visible") continue; const q = n.getBoundingClientRect(); if (q.left >= -1 && q.right <= vw + 1 && q.top >= -1 && q.bottom <= vh + 1) return true; } return false; };
  const vazamentos = [], miudos = [], contraste = [], semNome = [], todos = [];
  for (const el of RAIZ.querySelectorAll("*")) {
    ...
    if (visivel && (fora > 1 || foraV > 1) && !recortado(el)) {
      const pai = el.parentElement, rp = pai && pai.getBoundingClientRect();
      if (!rp || (rp.right - vw <= 1 && rp.left >= -1 && (cs.position !== "fixed" || (rp.bottom - vh <= 1 && rp.top >= -1)))) vazamentos.push(caminho(el) + " passa " + Math.round(fora > 1 ? fora : foraV) + "px " + (fora > 1 ? "na horizontal" : "na vertical"));
    }
  ...
  return { vazamentos, miudos, contraste, alvos, semNome, sobreposicoes };
})()`;

interface Medida { vazamentos: string[]; miudos: string[]; contraste: string[]; alvos: string[]; semNome: string[]; sobreposicoes: string[] }
  ...
  for (const largura of LARGURAS) {
    test(`páginas e painéis em ${largura}px`, async () => {
      ...
      for (const tema of ["light", "dark"] as const) {
        ...
        for (const [nome, caminho] of [["conexão", "/__anotador/"], ["extrator", "/__anotador/extrair"], ["microfone", "/__anotador/microfone"]] as const) {
